...
def Get_theta_dot_5_dof(x_target, y_target, z_target, vmax, joint_position_local):
    ...
    Goal_coordinate_5dof = np.array([float(x_target), float(y_target), float(z_target)])

    # coordinate_sixdof = Coordinate_fanuc(t1, t2, t3, t4)
    # x, y, z = coordinate_sixdof[-1, :3] + [x_transfer, 0, 0]

    pos = p.getLinkState(bodyUniqueId = Robot_2, linkIndex = 4)[0]
    x, y, z = pos

    att_5dof = [x, y, z] - Goal_coordinate_5dof
...
x_transfer = 0.8
p.resetBasePositionAndOrientation(Robot_1, [-0.05060699498278387 , 0.06190083623425062, 0], p.getQuaternionFromEuler([-np.pi/2, 0, 0]))
p.resetBasePositionAndOrientation(Robot_2, [ 0.0060575809083015 - 0.05395583 + 0.01070576 + x_transfer , 0.08475211989803633 - 0.02584229 + 0.029350807724098347 - 3.49770982e-03, 0], p.getQuaternionFromEuler([-np.pi/2, 0, 0]))
