...
class Robot6SacEnv(gym.Env):
    def __init__(self, use_gui: bool = False):
        ...
        # Tham số chuẩn hóa
        self.v_max = self.robot6.vel_limit  # Rad/s (Max physical joint vel)
        self.q_norm = np.pi
        self.pos_norm = 1.0
        ...
        # ===== GYM SPACES =====
        # Action: 7 chiều. 
        # [0-5]: RL Velocity (Tanh -> [-1, 1])
        # [6]:   Alpha (Sigmoid -> [0, 1])
        # Lưu ý: Gym Box low/high chỉ để tham khảo, Agent sẽ handle activation
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(7,), dtype=np.float32)

        # Observation: q6(6) + dq6(6) + e_pos(3) + observer(features) + u_base(6)
        # Observer: self.observer_points observer points; mỗi observer lấy self.observer_k
        # nearest neighbors trên robot5, mỗi neighbor trả về [nx, ny, nz, d_norm] (4 values)
        self.observer_points = 5
        self.observer_k = 1  # số neighbor gần nhất để bao gồm cho mỗi observer point

        obs_dim = 6 + 6 + 3 + (self.observer_points * self.observer_k * 4) + 6
        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=(obs_dim,), dtype=np.float32)

    # ... [Giữ nguyên các hàm draw_lines, set_target, get_joint_state] ...
    ...
    def _get_observer_features(self):
        ...
        for p6 in pts6:
            diffs = pts5 - p6
            dists = np.linalg.norm(diffs, axis=1)
            # minimal distance per observer point (kept for reward computation)
            d_min = float(np.min(dists))
            d_list.append(d_min)
            all_dists.append(d_min)

            # indices of k nearest neighbors on pts5
    ...
    def _get_obs(self):
        """
        Input state: 41 chiều
        [q6(6), dq6(6), e_pos(3), observer(20), u_base(6)]
        """
        # 1. Trạng thái khớp
        ...
        # 4. [NEW] U_Base (Tín hiệu điều khiển phi tuyến)
        # Tính toán u_base giả định tại trạng thái hiện tại
        joint_position_robot6 = self.robot6.take_joint_position()
        # Wrap angle nếu cần
        joint_position_robot6 = (joint_position_robot6 + np.pi) % (2 * np.pi) - np.pi
        
        # Gọi hàm tính toán của bộ điều khiển nền
        # Lưu ý: get_theta_dot trả về vận tốc khớp mong muốn (rad/s)
        ...
        # Chuẩn hóa u_base về [-1, 1] (chia cho v_max)
        u_base_norm = np.clip(np.array(u_base_vel) / self.v_max, -1.0, 1.0)

        # 5. Ghép lại thành vector 41 chiều
    ...
    def step(self, action):
        ...
        # 1. Giải mã Action (Hybrid Mixing)
        # Action có 7 phần tử. 
        # [0-5]: u_rl (đã qua Tanh ở Agent nên là [-1, 1])
        # [6]: alpha (đã qua Tanh ở Agent nên là [-1, 1], map sang [0, 1])
        
        u_rl_norm = action[0:6] 
        # Map alpha từ [-1, 1] → [0, 1] (symmetric & learnable)
        alpha = (action[6] + 1.0) / 2.0
        alpha = np.clip(alpha, 0.0, 1.0)  # Safety clip

        # 2. Lấy tín hiệu Base Controller (Tính lại để đảm bảo đồng bộ)
        joint_position_robot6 = self.robot6.take_joint_position()
        joint_position_robot6 = (joint_position_robot6 + np.pi) % (2 * np.pi) - np.pi
        ...
        # --- Xử lý Robot 5 (Vật cản) ---
        theta_dot_robot5 = [0]*5
        self.robot5.set_joint_velocity(theta_dot_robot5)
        ...
        # R1: Va chạm (dựa trên d_list)
        r1_list = []
        for d in d_list:
            if d >= self.d_safe: r = 1.0
            elif self.d_coll <= d < self.d_safe: r = (d - self.d_coll)/(self.d_safe - self.d_coll + 1e-6)
            else: r = -1.0
            r1_list.append(r)
        R1 = float(np.mean(r1_list))

        # R2: Giới hạn khớp
        R2 = 0.0 # (Giản lược cho ngắn, bạn có thể copy lại logic cũ nếu cần)

        # R3: Tới đích
        R3 = float(np.clip(1.0 - d_goal / (self.L_max + 1e-6), 0.0, 1.0))

        # R4: Phạt sử dụng RL (Alpha càng lớn càng phạt)
        # Logic: Khuyến khích alpha -> 0 khi an toàn
        R4 = - (alpha ** 2)
